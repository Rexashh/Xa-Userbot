...
@bot.on(rose_cmd(outgoing=True, pattern=r"waifu(?: |$)(.*)"))
async def waifu(animu):
    ...
    if not text:
        if animu.is_reply:
            ...
            await animu.answer("✖️ `Tidak ada teks yang diberikan, maka waifu melarikan diri.`")
            return
...
async def _(hazmat):
    ...
    if not hazmat.reply_to_msg_id:
        await hazmat.edit("🌹`WoWoWo Capt!, kita tidak akan cocok dengan hantu!...`")
        return
    reply_message = await hazmat.get_reply_message()
    if not reply_message.media:
        await hazmat.edit("🚧 `Kata bisa menghancurkan apapun Capt!...`")
        return
    chat = "@hazmat_suit_bot"
    await hazmat.edit("```Suit Up Capt!, ​​Kami akan membersihkan beberapa virus...```")
    ...
    async with hazmat.client.conversation(chat) as conv:
        ...
        if response.text.startswith("I can't"):
            await hazmat.edit("✖️ `Tidak dapat menangani GIF ini...`")
...
CMD_HELP.update({
    "waifu":
    f"**✘ Plugin waifu\
\n\n  •  Perintah : `{cmd}waifu` [teks]\
  \n  •  Fungsi : Custom Stickers.\
\n\n  •  Perintah : `{cmd}hz` [flip, X2, rotate (degree), background (number), black]\
  \n  •  Fungsi : Replay kefoto Atau Stiker."})
