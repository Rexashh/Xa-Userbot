...
@Xa_cmd(pattern="fgban(?: |$)(.*)")
async def gbun(event):
    if event.fwd_from:
        return
    gbunVar = event.text
    gbunVar = gbunVar[6:]
    mentions = f"`Globall Banned!! User 𝙂𝘽𝘼𝙉𝙉𝙀𝘿 By` {DEFAULTUSER}\n"
    no_reason = "No Reason Given "
    await event.edit("**𝘎𝘓𝘖𝘉𝘈𝘓 𝘉𝘈𝘕𝘕𝘌𝘋 𝘈𝘒𝘈𝘕 𝘚𝘌𝘎𝘌𝘙𝘈 𝘈𝘒𝘛𝘐𝘍...**")
    asyncio.sleep(3.5)
    ...
    if event.reply_to_msg_id:
        ...
        mention = (
            f"Global Banned!! User 𝙂𝘽𝘼𝙉𝙉𝙀𝘿 By {DEFAULTUSER} \nReason: No Reason Given. ")
