...
async def on_afk(event):
    ...
    if USER_AFK and not (await event.get_sender()).bot:
        ...
        if reason:
            message_to_reply = (
                f"**֍ {owner} 𝙎𝙚𝙙𝙖𝙣𝙜 𝘼𝙁𝙆** `{total_afk_time}` **𝙔𝙖𝙣𝙜 𝙇𝙖𝙡𝙪 ֎**\n"
                + f"**⋙ 𝙆𝙖𝙧𝙚𝙣𝙖 :** `{reason}`"
            )
        else:
            message_to_reply = (
                f"**֎ 𝙈𝘼𝘼𝙁 {owner} 𝙎𝙚𝙙𝙖𝙣𝙜 𝘼𝙁𝙆** `{total_afk_time}` **𝙔𝙖𝙣𝙜 𝙇𝙖𝙡𝙪 ֎**"
            )
        try:
            if pic.endswith((".tgs", ".webp")):
        ...
        if reason:
            ...
            try:
                if pic.endswith((".tgs", ".webp")):
                    await event.client.send_message(event.chat_id, file=pic)
                    await event.client.send_message(
                        event.chat_id, f"**֎ {owner} 𝙏𝙚𝙡𝙖𝙝 𝘼𝙁𝙆 ֎**"
                    )
                else:
                    await event.client.send_message(
                        event.chat_id,
...
CMD_HELP.update(
    {
        ...
        \n  •  **Function : **Memberi tahu kalau Master  𝙎𝙚𝙙𝙖𝙣𝙜 𝘼𝙁𝙆 bisa dengan menampilkan media keren ketika seseorang menandai atau membalas salah satu pesan atau dm Anda.\
        \n\n  •  **Syntax :** `{cmd}off`\
